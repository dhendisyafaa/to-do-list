...
const Beranda = () => {
  ...
  // bisa submit task walaupun form input task tidak terisi
  // bisa submit walaupun terdapat task yang sama
  // ketika menghapus task dari urutan atas (task pertama), checkbox terduplikasi
    ...
    <>
      ...
      <div className='mx-4 my-6'>
        ...
        <div className="border border-accent mt-4 mb-4"></div>
          ...
          <div className="mt-4">
            {tasks.map((todo) =>
              <div className='flex justify-between items-center border border-accent rounded-md p-3 mb-3 shadow-[3px_3px_0px_0px_#a61C4E]'>
                <div className='flex justify-between w-full'>
                  <p className='font-semibold' key={todo.id}>{todo.text}</p>
                  <div className='flex gap-2'>
                    <input type="checkbox" key={todo.id} onChange={() => toggleComplete(todo.id)}/>
                    <button
                    key={todo.id} 
